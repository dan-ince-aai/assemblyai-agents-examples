...
- `await conn.say(text)` sends a `conversation.message` (role user) and **the platform does not put it in the transcript** — measured, not assumed: a probe agent whose `llm=` pointed at a dumper received no `user` message at all. Only `create_reply(instructions=…)` reaches the endpoint, as a `system` message, and it is **gone from history by the next turn**; `assistant` lines are what persists. So a text-driven session has no caller turns: a stage needing a value the caller gave two turns ago will not find it, and the model will invent one — which is how a live call reached `verify(policy_number="policy number")`. Rehearse multi-turn calls offline instead (`starter/rehearse.py`); if you must drive a live one, have each stage read values back so the assistant's own lines carry them. Real speech is unaffected: it arrives as ordinary `user` messages. Note that a `reply.create` sent while the greeting is playing replaces the greeting. `conn.session` exposes the underlying `AsyncRealtimeSession`.
...
- `say(text)` / `call_tool(name, **arguments)` / `silence()` build the three answers, typed `Say`, `Call` and `Silence`. `call_tool` drops arguments whose value is `None` or `""`, because the platform refuses values the call never established; `established(**kwargs)` does that on its own.
- `stream(turn, answer, chunk_words=True)` yields the SSE lines; `json_body(turn, answer)` is the same answer as a plain body, for curl and tests. Pass them to whatever your framework wants.
- `digits_said(text)` — digits from figures or words, merging "forty one" into 41 and expanding "double one".
- There is no framework here, and nothing that imports a web library. How you organise the decision, and which routes you expose, is yours; `starter/flow.py` and `starter/backend.py` show one way.

Nothing in this package knows what a tunnel is. Tool URLs come from `PUBLIC_BASE_URL`; `expose.py` is the only file that starts one, and `one_file_agent.py` shows the order: address, then declaration, then deploy, then serve.
