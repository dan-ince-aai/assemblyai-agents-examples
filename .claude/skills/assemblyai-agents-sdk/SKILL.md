...
| control over what is said | your own replies (`llm=`) | `one_file_agent.py` |
| stages, or cost control, or a stage that provably cannot do certain things | subagent routing | `subagents.py` |
| a project rather than a script: tests, a system of record, a call flow to grow | the full kit | `starter/` |
...
```python
with public_address(PORT) as base_url:      # expose.py: ngrok
...
`starter/` is the same three steps in `run.py`, around a project. Four
...
1. **Install and check credentials.**
   ...
   The SDK is a package; the shapes above are files. `pip` gives you
   `assemblyai_agents` and nothing else, and every shape does
   `from expose import public_address`, which is a file in this repo. So clone
   this repo and copy the shape you want plus `expose.py` into the project:
   ```bash
   git clone https://github.com/dan-ince-aai/assemblyai-agents-examples.git
   cp assemblyai-agents-examples/{tools_only_agent.py,expose.py} my-agent/
   ```
   `starter/` ships its own `expose.py`, so copying that directory is enough on
   its own. If you are already working inside this repo, there is nothing to
   copy — run the file where it is.
...
4. **Serve it** with `serving.serve(agent, reply=..., tool_secret=..., ...)`,
   ...
   resolve`), so a placeholder cannot be deployed. `expose.py` starts
...
6. **Verify** in this order, cheapest first:
   ...
   - Rehearse the whole call offline: run the reply engine and the tools in a
     loop with no platform at all, which is what `starter/rehearse.py`
...
`expose.py` is the address half: it starts ngrok unless
...
`tools_only_agent.py` is that in a single file, with the platform's
own model running the conversation and the script answering only tool calls.
`one_file_agent.py` is the same with `llm=` added, so the script
decides every reply too. `starter/run.py` is the same three steps for
...
`subagents.py` routes a call between stages, each with its own model,
...
- Check a change without picking up the phone by rehearsing the call offline:
  ...
  as `starter/rehearse.py` does and its tests drive. Milliseconds per
...
whole agent is one script: see `one_file_agent.py`, which declares its
...
The public address comes from `PUBLIC_BASE_URL`, or from `expose.py`,
...
staging host, a deployment later. `expose.py` is the only file that
starts one.
...
loop); and deliver a settled position. `starter/model.py` is those
...
`one_file_agent.py`. If the project already has a web application,
